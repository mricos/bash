...
TILE_NAME_TO_CHAR["ERROR"]="X" # Map internal error name to char
...
propagate() {
    ...
    while (( ${#queue[@]} > 0 && processed_count < max_processed )); do
        ...
        local original_options_names="${grid[$current_key]-}"; local original_options_chars="${possibilities[$current_key]-}"
        if [[ "$original_options_names" == "$ERROR_SYMBOL" ]]; then continue; fi

        local -a surviving_names_arr=($original_options_names); local possibilities_reduced=0
        ...
        for dir in "${directions[@]}"; do
            ...
            local neighbor_name="${grid[$nkey]-}"; if [[ "$neighbor_name" == "$ERROR_SYMBOL" ]] || [[ -z "$neighbor_name" ]]; then continue; fi

            local rule_key="${neighbor_name}_${opposite_dir}"; local allowed_names_by_neighbor=" ${rules[$rule_key]-} "
            local -a next_surviving_names_arr=()
            for potential_name in "${surviving_names_arr[@]}"; do [[ "$allowed_names_by_neighbor" == *" $potential_name "* ]] && next_surviving_names_arr+=("$potential_name"); done

            if (( ${#surviving_names_arr[@]} != ${#next_surviving_names_arr[@]} )); then possibilities_reduced=1; surviving_names_arr=("${next_surviving_names_arr[@]}"); if (( ${#surviving_names_arr[@]} == 0 )); then break; fi; fi
        done

        if (( ${#surviving_names_arr[@]} == 0 )); then # Contradiction
        ...
        if [[ $possibilities_reduced -eq 1 ]]; then
             ...
             if [[ "$original_options_names" != "$new_options_names" ]]; then # Check names changed
